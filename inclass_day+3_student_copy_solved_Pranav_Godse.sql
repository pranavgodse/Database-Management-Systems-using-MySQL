...
select Month,sum(Purchase_Cost), avg(Estimated_Sale_Price)
...
select sum(ifnull(purchase_cost,2000)) as sum_of_purchase_cost
from bank_inventory_pricing;
...
select avg(ifnull(purchase_cost,estimated_sale_price)) as avg_of_purchase_cost
from bank_inventory_pricing;
...
select product, sum(Purchase_cost) as Sum_of_Purchase_Cost
from bank_inventory_pricing
...
select product, avg(Purchase_cost) as Avg_of_Purchase_Cost
from bank_inventory_pricing
group by product
...
select product, avg(estimated_sale_price) as Original_Price, avg(estimated_sale_price) * 1.15 as New_Price
from bank_inventory_pricing
group by product
...
# Method 1:
select avg(ifnull(purchase_cost,if(price>estimated_sale_price,price,estimated_sale_price))) as average
from bank_inventory_pricing
where product = 'BusiCard';

# Method 2:
select avg(ifnull(purchase_cost, greatest(price,estimated_sale_price))) as average
from bank_inventory_pricing
where product = 'BusiCard';
...
select product, avg(ifnull(Estimated_Sale_Price,Purchase_Cost)) as average_estimated_sale_price
from bank_inventory_pricing
group by product;
