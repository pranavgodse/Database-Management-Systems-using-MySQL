...
select ba.customer_id,ba.account_number,ba.account_type,
sum(case when dayname(t.transaction_date)='Friday'
then t.transaction_amount end) as Friday_transaction_amount,

sum(case when dayname(t.transaction_date)!= 'Friday'
then t.transaction_amount end) as other_transaction_amount
...
select * from bank_account_transaction;
select
ba.customer_id,ba.account_number,ba.account_type,t.transaction_da
te,h.holiday,
sum(case when t.transaction_date = h.holiday then
t.transaction_amount else 0 end ) holiday_transaction,
sum(case when t.transaction_date = h.holiday then 0 else
t.transaction_amount end ) non_holiday_transaction
from bank_account_details ba
join bank_account_transaction t
on ba.account_number = t.account_number
left join bank_holidays h
on t.transaction_date = h.holiday
where ba.account_type = 'Credit Card'
group by 1,2,3,4,5;
...
select * from bank_account_transaction;
select * from bank_holidays;
select t.account_number, t.transaction_date,h.holiday,
case when m.event='adhoc' then 'Happy Holiday' end as
Holiday_Greeting
from bank_account_transaction t
join bank_holidays h
on t.transaction_date = h.holiday
...
select
ba.customer_id,ba.account_number,ba.account_type,t.transaction_amount,
month(t.transaction_date)-i.month as num_of_months,
t.transaction_amount*i.interest_rate as accrued_interest
from bank_account_details ba
join bank_account_transaction t
on ba.account_number = t.account_number
join bank_interest_rate i
on i.account_type=ba.account_type
where ba.account_type = 'recurring deposits' and
month(t.transaction_date)-i.month>1 and t.transaction_amount>0;

select ba.account_type,month(bt.transaction_date)-bi.month as
no_of_month ,bt.transaction_amount*bi.interest_rate Interest
from bank_account_details ba,bank_account_transaction
bt,bank_interest_rate bi
where ba.account_type='Recurring Deposits' and
month(bt.transaction_date)-bi.month>1;
select * from bank_interest_rate;
select * from bank_account_details;
select month(transaction_date) from bank_account_transaction;
...
select ba.account_number as primary_account_number,
 ba.account_type as primary_account_type,
 br.account_number as secondary_account_number,
 br.account_type as secondary_account_type,
 count(case when br.account_number = t.account_number then
t.account_number end) num_of_transaction
from bank_account_details ba
join bank_account_relationship_details br
on br.linking_account_number = ba.account_number
join bank_account_transaction t
on t.account_number = br.account_number
where ba.account_type = 'savings' and br.account_type in ('credit
card' , 'Add-on Credit Card')
group by 1,2,3,4
having num_of_transaction>1;

SELECT
 ba.account_number AS Primary_account,
 ba.account_type AS Primary_Account_type,
 br.account_number AS Secondary_account,
 br.account_type AS Secondary_Account_type
FROM
 bank_account_details ba
 JOIN
 bank_account_relationship_details br ON ba.account_number =
br.Linking_Account_Number
 JOIN
 bank_account_transaction at ON br.Account_Number =
at.Account_Number
WHERE
 ba.Account_type = 'Savings'
 AND br.Account_type IN ('credit card' , 'Add-on Credit
Card')
...
select ba.account_number as primary_acc_number,
 ba.account_type as primary_acc_type,
 br.account_number as secondary_acc_number,
 br.account_type as secondary_acc_type,
 count(case when br.account_number =t.account_number then
t.account_number end) as number_of_transaction
from bank_account_details ba
join bank_account_relationship_details br
on br.linking_account_number = ba.account_number
join bank_account_transaction t
on t.account_number = br.account_number
where br.account_type = 'Add-on Credit Card'
group by 1,2,3,4
...
WHERE
 ba.Account_type = 'Savings'
 AND br.Account_type = 'Add-on Credit Card'
GROUP BY at.Account_Number
HAVING COUNT(*) >= 1;
...
select * from bank_customer bc
join bank_customer_export bce
on cast(bc.customer_id as unsigned) = bce.customer_id;

SELECT *
FROM bank_customer bc
LEFT JOIN bank_customer_export bce ON bc.customer_id =
bce.customer_id
UNION
SELECT * FROM bank_customer bc
RIGHT JOIN bank_customer_export bce ON bc.customer_id =
bce.customer_id;
