...
select branch, sum(estimated_profit) as total_profit
from bank_branch_pl
where estimated_profit > (revenue - cost)
group by branch;
...
select product, avg(estimated_sale_price) as Average_Estimated_Sale_Price
...
select product, month, sum(quantity) as 'total quantity'
from bank_inventory_pricing
...
/* Solution */
# Method 1:
select product, (Estimated_sale_price-Purchase_cost) as 'maximum profit'
from bank_inventory_pricing
where (Estimated_sale_price-Purchase_cost) > 0
group by product; 

# Method 2:
alter table bank_inventory_pricing 
add column maximum_profit float;

update bank_inventory_pricing
set maximum_profit = Estimated_sale_price-Purchase_cost;

select product,max(maximum_profit) as 'maximum profit'
from bank_inventory_pricing
where maximum_profit > 0
group by product;
...
select banker, min(revenue) as 'minimum revenue'
from bank_branch_pl
...
add column Real_profit float;

update bank_branch_pl
set Real_profit = revenue - cost;

select branch, real_profit
from bank_branch_pl
...
select branch, sum(Estimated_profit) as 'total estimated profit', real_profit
from bank_branch_pl
where real_profit >= 0;
...
select branch, product, real_profit
from bank_branch_pl
where real_profit > 0
order by real_profit;

# Question 11:
# 11)  Calculate the branch and product level total calculated profit where the calculated profit is more than the 
# estimated profit after ignoring losses.

select branch, product, count(branch) as branch_count, count(product) as product_count
from bank_branch_pl
where real_profit != 0 and real_profit > Estimated_profit;
...
select product
from bank_branch_pl
where (real_profit = Estimated_profit)-(Estimated_profit*(10/100));
...
alter table bank_branch_pl add column reduced_cost float;

alter table bank_branch_pl add column reduced_real_profit float;

update bank_branch_pl
set reduced_cost = cost-(cost*(30/100));

update bank_branch_pl
set reduced_real_profit= revenue - reduced_cost;

alter table bank_branch_pl drop column reduced_cost ;

alter table bank_branch_pl drop column reduced_real_profit ;
